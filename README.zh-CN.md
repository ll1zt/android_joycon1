...
└──────────────┘                                    └────────────────────┘ │
                                  ...
                                  uinput 合成设备 0x2008    │               │
                                  标准手柄轴(X/Y/Z/RZ/HAT…) ▼               │
                                  ◄─── service.sh 自挂载  /system/usr/{keylayout,idc}
                                                          │               │
                          FF upload/play ─────────────────┤               │
                                                          ▼               │
...
1. **内核层**:新 GKI 内核自带 `CONFIG_HID_NINTENDO=y`(编进本体),Joy-Con 直接绑
   ...
   `CONFIG_NINTENDO_FF`(力反馈子开关)——内核 FF 接口照常暴露,却永远不会把震动
   发给手柄。本项目的解法是**绕开**它,见第 2 层。
...
3. **框架层**:keylayout(`Vendor_057e_Product_2008.kl`,LineageOS 2025 版,含 HAT
   轴 + 模拟扳机)与 idc 文件由模块的 `service.sh` bind-mount 进
   `/system/usr/{keylayout,idc}`。三个坑都在这里踩过:KernelSU Next ≥ 3.3 没有
   metamodule 就没有 magic mount(于是脚本自己挂);`/data` 分区无法直接 exec
   (守护进程先拷到 `/dev` tmpfs 再跑);SELinux enforcing 拒绝 system_server 读
   tmpfs 文件(`chcon u:object_r:system_file:s0` 修正标签)。
...
| 单只手柄从框架隐藏(idc `device.disabled=1`) | ✅ |
| 普通震动(strong→LF 低频带、weak→HF 高频带) | ✅ hidraw 直通 |
...
adb shell 'dumpsys input | grep -A8 Combined'       # 框架视角
```
